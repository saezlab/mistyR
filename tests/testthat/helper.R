...
generate_random_tibble <- function(rows, columns) {
  means <- runif(columns, 1, 10)
  seq_len(columns) %>%
    purrr::map_dfc(~ tibble::as_tibble_col(rnorm(rows, means[.x], log(means[.x])),
      column_name = paste0("expr", .x)
    ))
}

# sample without replacement from a grid.rows * grid.columns grid
sample_grid_geometry <- function(rows, grid.rows, grid.columns) {
