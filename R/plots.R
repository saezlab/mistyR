...
#' @param measure performance measure to be plotted (See \code{\link{collect_results()}}).
#'
...
#' 
#' Identify and plot a graph of marker interaction communities. The communities
#' are identified using the Louvain algorithm. Communities can be extracted only
#' from views that have the same predictor and target markers.
#'
#' @inheritParams plot_interaction_heatmap
#' 
#' @return The \code{misty.results} list (invisibly).
...
#'
#' Plot interexperiment contrast of views. The heatmaps show the interactions 
#' that are present and have importance above a \code{cutoff.to} value in the 
#' \code{views} of \code{misty.results.to} but not present or have importance 
#' below \code{cutoff.from} in the \code{views} of \code{misty.results.from}.
#' 
