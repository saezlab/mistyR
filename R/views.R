...
#'     If not provided (\code{unique.id = NULL}) then an id is automatically generated
#'     by calculating the md5 hash of \code{table}.
#'
#' @return An initial MISTy view composition containing an \emph{intraview} list item named described
#'     with abbreviation "intra" and \emph{data} as provided in \code{data} and a \emph{misty.uniqueid}
#'     list item containing the provided or automatically calculated \code{unique.id}.
#'     A cache folder for the sample will be automatically created in the working directory
#'     as a subfolder of \file{.misty.temp/} with the same name as \code{unique.id}.
#'
...
#' @param data A \code{data.frame} or a \code{tibble} with named variables in columns
#'     and rows for each spatial unit ordered as in the intraview.
#' @param abbrev Abbreviated name. A character vector.
#'
#' @return A new MISTy view. A list with a single \code{name}d item described by the provided \code{abbrev}iation
#'     and \emph{data} containing the provided \code{data}.
#'
#' @seealso \code{\link{add_views()}} for adding created views to a view composition.
#'
#' @family view composition functions
#'
#' @export
...
#'     with an intraview, \code{\link{create_view()}} for creating a custom view.
#'
#' @family view composition functions
#'
#' @export
...
#'
#' @export
add_paraview <- function(current.views, positions, l, approx = 1, nn = NULL, cached = TRUE, verbose = TRUE) {
