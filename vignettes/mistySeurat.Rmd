...
`r Githubpkg("saezlab/mistyR")` can be used to analyze spatial transcriptomics data sets stored in `r CRANpkg("SeuratObject")` with just a couple of functions. In this vignette we demonstrate how to build a user friendly workflow starting from data preprocessing, through running `r Githubpkg("saezlab/mistyR")`, to analysis of results, i.e., the spatial interactions between markers stored in alternative assays and specific locations.

In principle `r Githubpkg("saezlab/mistyR")` can be used with data extracted from a `r CRANpkg("SeuratObject")` only. For the sake of completeness of the process of modeling with `r Githubpkg("saezlab/mistyR")` we demonstrate a complete workflow, including data preprocessing with `r CRANpkg("Seurat")`. The functions provided in this notebook can be adapted to the user preference but the main objective is to exploit as much as possible the flexibility of workflow creation from `r Githubpkg("saezlab/mistyR")` and object manipulation from `r CRANpkg("Seurat")`.
...
For user convenience and to facilitate the use of `r Githubpkg("saezlab/mistyR")` and `r CRANpkg("Seurat")`, `run_misty_seurat()` is a function describing a general skeleton of a mistyR workflow for analysing a 10x Visium slide given in a Seurat object. The function allows for:
...
run_misty_seurat <- function(visium.slide,
                             ...
                             view.features = NULL,
                             # Named list of features/markers to use. Use all by default.
                             view.types,
                             # Named list of the type of view to construct from the assay.
                             view.params,
                             # Named list containing the parameters (NULL or value) for each view.
                             spot.ids = NULL,
  ...
  # Extracting geometry
  geometry <- GetTissueCoordinates(visium.slide, cols = c("row", "col"), scale = NULL)
...
These are helper functions that allow to extract from `r CRANpkg("Seurat")` objects specific assays and transform them into `tibble` which a preferred format for `r Githubpkg("saezlab/mistyR")`.
...
This helper function wraps the three available options for view creation in `r Githubpkg("saezlab/mistyR")` with additional features that allow for creating views for specific spots.
...
Finally, we show a wrapper function `build_misty_pipeline()` that allows you to build automatically a `r Githubpkg("saezlab/mistyR")` workflow from a list of data frames, with specified spatial context, parameters, features and locations.
...
if(!dir.exists("breast_A_1")){
  download.file("https://www.dropbox.com/s/igby4csbt9u4uuf/breast_A_1.tgz?dl=1",
                destfile = "breast_A_1.tgz", quiet = TRUE)
  untar("breast_A_1.tgz", tar = "internal")
  cleanup <- TRUE
}
```

We will explore the spatial interactions of the Hypoxia pathway responsive genes with the Estrogen pathway responsive genes. To this end we will use the model matrix with top significant genes for each pathway from the package `r Biocpkg("progeny")` and the previously described functions.
...
For this simple example we will pick the top 15 most significantly responsive genes of each pathway from the model matrix from the package `r Biocpkg("progeny")`.
...
Now that we have preprocessed the data and have decided on a question to analyze, we can create and run a `r Githubpkg("saezlab/mistyR")` workflow.
...
In the case of PGK1, we observe that around 50% of the contribution in the final model comes from the expression of other markers of hypoxia intrinsically or from the broader tissue structure. The rest (46%) comes from the expression of estrogen responsive genes from the broader tissue structure.
...
-   1-to-1 importances between predictor and markers should always be interpreted in the context of the other predictors, since MISTy is defined as a multivariate predictive task.
...
Additionally, `r Githubpkg("saezlab/mistyR")` through the function `collect_results()` allows you to group the results of multiple slides, allowing for a more robust, integrative or comparative analysis of spatial interactions.
