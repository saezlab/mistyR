...
`mistyR` can be used to analyze spatial transcriptomics data sets stored in `Seurat` objects with just a couple of functions. In this vignette we will show how to build user friendly workflows to analyze the spatial interactions between markers stored in alternative assays and specific locations.

The functions provided in this notebook can be adapted to the user preference but the main objective is to exploit as much as possible the flexibility of workflow creation from `mistyR` and object manipulation from `Seurat`.
...
For user convenience and to facilitate the use of `mistyR` and `Seurat` `run_misty_seurat` is a function decribing a general skeleton of a mistyR workflow for analysing a 10x Visium slide given in a Seurat object. The function allows for
...
These are helper functions that allow to extract from `Seurat` objects specific assays and transform them into `data.frame` which is the standard format in `mistyR`
...
This helper function wraps the three available options for view creation in `mistyR` with additional features that allow you to create views for specific spots.
...
We will explore the spatial interactions of the Hypoxia pathway responsive genes with the Estrogen pathway responsive genes. To this end we will use the model matrix with top significant genes for each pathway from the package `progeny` and the functions defined above.
